...
class FileBrowser:
    def __init__(self, directory, device):
        ...
        # Recherche les images dans le dossier
        exts = ["*.jpeg", "*.jpg", "*.png", "*.JPEG", "*.JPG", "*.PNG"]
        for ext in exts:
            self.files.extend(glob.glob(os.path.join(self.directory, ext)))

        # Si aucune image → ouvrir une fenêtre pour importer
        if not self.files:
            print(f"[INFO] Aucune image trouvée dans {self.directory}. Veuillez sélectionner des images.")
            self.select_images()
...
class BioticSegmentation:
    ...
    def load_images(self, directory=None):
        ...
        if len(self.image_dataset) == 0:
            print("[INFO] Aucun fichier à traiter, le programme continue mais sans images.")
            return
    ...
    def init_ui(self):
        self.root = tk.Tk()
        self.root.title("SAMourAI - Image Segmentation Tool")
        ...
        if os.path.exists(icon_path):
            try:
                ...
                print(f"[WARNING] Impossible de charger l'icône : {e}")
        else:
            print("[WARNING] Fichier d'icône introuvable : {icon_path}")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python SAMourAI.py <directory> <modelname>")
        sys.exit(1)
